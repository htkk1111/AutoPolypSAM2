...
def load_state_dict_into_model(
    ...
    def load_state_dict_into_model(
        ...
        """
        Loads a state dict into the given model, initializing new modules and loading existing weights.

        Args:
            state_dict: A dictionary containing the model's state dict.
            model: Model to load the checkpoint weights into.
            strict: Raise if the state_dict has missing state keys.
            ignore_missing_keys: Unix pattern of keys to ignore.
        """
        ...
        for name, module in model.named_modules():
            if isinstance(module, SAM2Base):  # 替换为你的新模块类
                init.kaiming_uniform_(module.weight, a=0.0, mode='fan_in', nonlinearity='relu')  # 进行凯明初始化
